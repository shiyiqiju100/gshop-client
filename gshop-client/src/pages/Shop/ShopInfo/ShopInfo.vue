  ...
  <div class="shop-info">
    <div class="info-content">
      ...
      <section class="section">
        ...
        <div class="activity">
          <div class="activity-item" :class="activeClasses[support.type]" v-for="(support,index) in info.supports"
               :key="index">
...
<script>
  ...
  export default {
    ...
    mounted() {  //从别的组件点击过来,如果info还没有内容,直接返回
      if(!this.info.name){ //因为info默认是{},不能(!this.info)这样判断,只能判断它里面是否有内容
        return
      }
      // 从别的路由切换过来
        this. _initScroll()
    ...
    methods:{
      _initScroll(){
        ...
        //因为ul的宽度和外部容器的宽度一样,导致滑动不了
        //动态指定ul的宽度
        const ul = this.$refs.picsUl
        const liWidth = 120
        const space = 6 //li之间有6px的margin-right
